    ...
    for produto in produtos:
        if "vencimentos" in produto:
            produto["vencimentos"] = [
                ...
                if (datetime.strptime(data_str, "%d/%m/%Y").date() + timedelta(days=7)) >= hoje
            ]
...
def remover_produto():
    ...
    if selecionado:
        ...
        confirmar = messagebox.askyesno("Confirmação", f"Tem certeza que deseja remover o produto '{produto['nome']}'?")
        if confirmar:
...
def adicionar_datas_ao_produto():
    selecionado = lista_produtos.curselection()  # <-- MANTER AQUI, ANTES DE ATUALIZAR
    if not selecionado:
        popup_aviso("Aviso", "Selecione um produto primeiro.")
        return

    indice_visual = selecionado[0]
    if indice_visual >= len(indice_mapeamento):
        popup_aviso("Erro", "Produto inválido selecionado. Atualize a lista e tente novamente.")
        return
...
def alternar_ordenacao():
    global modo_ordenacao
    if modo_ordenacao == "vencimento":
        modo_ordenacao = "alfabetica"
        popup_aviso("Modo de Ordenação", "Ordenação alterada para alfabética.")
    else:
        modo_ordenacao = "vencimento"
        popup_aviso("Modo de Ordenação", "Ordenação alterada para vencimento.")
    atualizar_lista_visual()
        ...
        if datas:
            ...
            for data_str in datas:
                try:
                    ...
                    datas_validas.append(data_venc)
                except:
                    continue
            if datas_validas:
                return min(datas_validas)
        return datetime.max.date()  # Produtos sem data vão para o final

    produtos_ordenados = sorted(
        enumerate(produtos),
        key=lambda p: chave_ordenacao(p[1])
    )
...
def aplicar_filtro(termo):
    lista_produtos.delete(0, tk.END)
    indice_mapeamento.clear()  # <-- ESSENCIAL PARA MAPEAR APÓS FILTRO
    ...
    if modo_ordenacao == "vencimento":
        produtos_ordenados = sorted(
            enumerate(produtos),
            key=lambda p: (prioridade(p[1]), p[1].get("nome", "").lower())
        )
    else:
        produtos_ordenados = sorted(
            enumerate(produtos),
            key=lambda p: p[1].get("nome", "").lower()
        )
    ...
    for idx_real, produto in produtos_ordenados:
        ...
        if campo == "nome":
            ...
            if termo in produto.get("código de barras", "").lower():
                incluir = True
...
def criar_interface():
    global janela, lista_produtos, entrada_busca, filtro_var
    ...
    lista_produtos = tk.Listbox(frame_lista, bg="white", fg="black", font=("Arial", 12), bd=0, highlightthickness=0)
    lista_produtos.pack(side="left", fill="both", expand=True)

    scrollbar = tk.Scrollbar(frame_lista, orient="vertical", command=lista_produtos.yview)
